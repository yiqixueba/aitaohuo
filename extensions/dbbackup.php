...
class dbbackup
{
    ...
    private function getTableCreateSql($table)
    {
        ...
        $tbname = $rs[0]['Table'];
        $string = str_replace('CREATE TABLE "' . $tbname . '"', 'CREATE TABLE IF NOT EXISTS "' . $tbname . '"', $string);
        $string = str_replace('"', '`', $string);
        $strinstart = "--" . $this->ln;
        $strinstart .= "-- 转储表中的数据" . $table . $this->ln;
        $strinstart .= "--" . $this->ln;
        $strinstart .= $this->ln;
        mysql_select_db('information_schema', $this->conn) or spError("无法链接数据库或者没有" . $this->database . "数据库访问权限。");
        $sql = "SELECT * FROM `TABLES` where TABLE_SCHEMA = '$this->database' and TABLE_NAME = '$tbname'";
        $rs = $this->getArray($sql);
        $charset = explode('_', $rs[0]['TABLE_COLLATION']);
